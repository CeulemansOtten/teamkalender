...
const TABLE_LEFT_PAD_PX = 10        // nieuwe linkerkolom
const FD_DATE_COL_PX = 160           // originele datumkolom Feestdagen
const SV_RANGE_COL_PX = 220          // originele datum-bereik kolom School
...
function isoToDDMMYYYY(iso: string | null): string | null {
  if (!iso) return null
  const parts = iso.split("-"); if (parts.length !== 3) return null
...
function ConfirmModal({
  open, title, message,
  confirmLabel = "Verwijderen", cancelLabel = "Annuleren",
  onConfirm, onCancel,
}: {
  open: boolean
  title: string
  ...
  return (
    <div role="dialog" aria-modal="true"
      ...
      <div onClick={(e)=>e.stopPropagation()}
        style={{ width:"min(420px, 90vw)", background:"#fff", borderRadius:12, border:`1px solid ${COLORS.line}`, boxShadow:"0 10px 30px rgba(0,0,0,0.2)", padding:16, fontFamily:SYS_FONT }}>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
          ...
          <button onClick={onConfirm} style={{ height:36, padding:"0 14px", borderRadius:8, border:`1px solid ${COLORS.danger}`, background:COLORS.danger, color:"#fff", cursor:"pointer", fontWeight:800 }}>
            {confirmLabel}
          </button>
        </div>
      </div>
...
export default function AdminPage() {
  ...
  /* ---------- Admin2: Uren ---------- */
  const [selectedPersonId, setSelectedPersonId] = useState("") // start met "-" (leeg)
  ...
  /* ================== Data laden ================== */
  // Personeel + statusopties
  useEffect(() => {
    const loadPeople = async () => {
      setLoadingPeople(true); setPErr("")
      const { data, error } = await supabase
        .from("personnel")
        .select("id, name, surname, avatar_url, status, birthdate")
        .order("name", { ascending: true })
      if (error) {
        setPErr(error.message || "Kon personeel niet laden."); setPeople([])
      } else {
        const list = (data || []).map((p: any) => ({ ...p, birthdate: ddmmyyyyToISO(p.birthdate ?? null) || "" })) as Personnel[]
        setPeople(list)
        // NIET automatisch iemand selecteren — dropdown begint met "-"
        setSelectedPersonId("")
        setYearFilter("")
        setYearOptions([])
      }
    ...
    const loadStatusOptions = async () => {
      ...
      if (error) setStatusOptions(["active", "inactive"])
        ...
        const values = (data || []).map((r: any) => r.status).filter((v: any) => typeof v === "string" && v.trim().length > 0)
  ...
  // Jaaropties voor geselecteerde persoon
 useEffect(() => {
  const fetchYears = async () => {
    ...
    if (unique.length) {
      if (unique.includes(currentYear)) {
        // Huidig jaar bestaat → kies dat
        setYearFilter(currentYear)
      } else {
        // Anders het laatste beschikbare jaar
    ...
    // Optioneel handig: bij uren toevoegen standaard huidig jaar invullen
    setNewYearInput(String(currentYear))
  }
  fetchYears()
}, [selectedPersonId])

  // Urenregels voor persoon + jaar
  useEffect(() => {
    const fetchEnts = async () => {
      if (!selectedPersonId || !yearFilter) { setEnts([]); return }
      setLoadingEnts(true); setEErr("")
      const { data, error } = await supabase
        .from("leave_entitlements")
      ...
      if (error) { setEErr(error.message || "Kon uren niet laden."); setEnts([]) }
  ...
  // Feestdagen voor yearPublic
  useEffect(() => {
    const fetchPublic = async () => {
      setLoadingPublic(true)
      const { data, error } = await supabase.from("holidays")
        .select("id, holiday_date, name, type")
        .eq("type", "public").gte("holiday_date", rangePublic.from).lte("holiday_date", rangePublic.to)
  ...
  // Schoolvakanties voor yearSchool
  useEffect(() => {
    const fetchSchool = async () => {
      setLoadingSchool(true)
      const { data, error } = await supabase.from("holidays")
        .select("id, holiday_date, name, type")
        .eq("type","school").gte("holiday_date", rangeSchool.from).lte("holiday_date", rangeSchool.to)
        .order("holiday_date", { ascending: true })
  ...
  // ★★★ HIER AANGEPAST: eerst leave_requests, dan leave_entitlements, dan person ★★★
  ...
  const doClearAvatar = async (p: Personnel) => {
    ...
    if (error) { setPErr(error.message || "Avatar verwijderen mislukt."); return }
  ...
  const handleAvatarFileChange = async (p: Personnel, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (upErr) { setPErr(upErr.message || "Upload mislukt."); return }
    const { data: pub } = supabase.storage.from("avatar").getPublicUrl(filePath)
    const url = pub?.publicUrl || ""
    if (!url) { setPErr("Kon publieke URL niet ophalen."); return }
    const { error } = await supabase.from("personnel").update({ avatar_url: url }).eq("id", p.id)
    if (error) { setPErr(error.message || "Updaten van avatar mislukt."); return }
  ...
  const addPerson = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.from("personnel")
      ...
      .select("id, name, surname, avatar_url, status, birthdate").single()
    ...
    if (data) {
      ...
      setNewName(""); setNewSurname(""); setNewBirthdate("")
    }
  }
  ...
  const addHours = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.from("leave_entitlements")
      .insert({ personnel_id: selectedPersonId, year: y, total_hours: hours, reason: newReason } as never)
      .select("id, personnel_id, year, total_hours, reason").single()
  ...
  const onAddPublic = async (e: React.FormEvent) => {
    ...
    if (new Date(fdDate).getFullYear() !== yearPublic) { setFdError(`Datum moet in ${yearPublic} liggen.`); return }
    setFdSaving(true)
    ...
    const { data } = await supabase.from("holidays").select("id, holiday_date, name, type")
      .eq("type","public").gte("holiday_date", rangePublic.from).lte("holiday_date", rangePublic.to).order("holiday_date", { ascending: true })
  ...
  const onAddSchool = async (e: React.FormEvent) => {
    ...
    if (new Date(svStart).getFullYear() !== yearSchool || new Date(svEnd).getFullYear() !== yearSchool) {
      setSvError(`Periode moet in ${yearSchool} liggen.`); return
    }
    setSvSaving(true)
    ...
    const { data } = await supabase.from("holidays").select("id, holiday_date, name, type")
      .eq("type","school").gte("holiday_date", rangeSchool.from).lte("holiday_date", rangeSchool.to).order("holiday_date", { ascending: true })
  ...
  const removeSchoolGroup = async (name: string) => {
    await supabase.from("holidays").delete().eq("type","school").eq("name", name).gte("holiday_date", rangeSchool.from).lte("holiday_date", rangeSchool.to)
    setSchoolHolidays((prev) => prev.filter(h => !(h.type==="school" && h.name===name)))
  }

  /* ================== Confirm modal handlers ================== */
  const askDeleteAvatar = (p: Personnel) =>
    setConfirm({ open:true, kind:"avatar", title:"Avatar verwijderen?", message:`Weet je zeker dat je de foto van ${p.name}${p.surname ? " " + p.surname : ""} wilt verwijderen?`, confirmLabel:"Verwijder avatar", person:p })
  const askDeletePerson = (p: Personnel) =>
    setConfirm({ open:true, kind:"person", title:"Personeelslid verwijderen?", message:`Als je ${p.name}${p.surname ? " " + p.surname : ""} verwijdert, gaan ook zijn/haar aanvragen en uren weg. Weet je het zeker?`, confirmLabel:"Verwijder persoon", person:p })
  const askDeleteHours = (ent: LeaveEntitlement) =>
    setConfirm({ open:true, kind:"hours", title:"Urenregel verwijderen?", message:`Je staat op het punt een urenregel (${ent.total_hours}u${ent.reason ? ` – ${ent.reason}` : ""}, ${ent.year}) te verwijderen. Doorgaan?`, confirmLabel:"Verwijder uren", ent })
  ...
  return (
    <>
      ...
      >
        {(pErr || eErr) && (
          <div role="status"
            style={{ margin:"0 auto", width:"min(1000px, 96vw)", background:"#fef2f2", color:"#7f1d1d", border:"1px solid #fecaca", borderRadius:10, padding:"10px 12px" }}>
            {pErr || eErr}
          </div>
        )}
        ...
        >
          {/* --------- LINKS – Uren beheren --------- */}
          <div style={{ width:`min(${HOURS_CARD_MAX_WIDTH}px, 100%)`, background:COLORS.card, border:`1px solid ${COLORS.line}`, borderRadius:12, padding:16, display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {!selectedPersonId ? (
              ...
              <ul style={{ listStyle:"none", padding:0, margin:0 }}>
                {ents.map((ent) => (
                  <li key={ent.id}
                    style={{ display:"grid", gridTemplateColumns:`${HOURS_FIELD_WIDTH_PX}px ${HOURS_FIELD_WIDTH_PX}px auto`, gap:8, alignItems:"center", borderBottom:`1px solid ${COLORS.line}`, padding:"8px 0" }}>
                    <input value={String(ent.total_hours)} onChange={(e)=>updateEntField(ent.id, "total_hours", e.target.value)} inputMode="decimal" style={{ ...baseField, width:HOURS_FIELD_WIDTH_PX }} />
                    ...
                    <div style={{ display:"flex", gap:6, justifyContent:"flex-end" }}>
                      <button aria-label="Opslaan" title="Opslaan" onClick={()=>saveEnt(ent)}
                        style={{ background:"transparent", border:`1px solid ${COLORS.line}`, height:32, width:32, borderRadius:8, display:"inline-flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                      ...
                      <button aria-label="Verwijderen" title="Verwijderen" onClick={()=>askDeleteHours(ent)}
                        style={{ background:"transparent", border:"none", height:32, width:32, borderRadius:8, display:"inline-flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                        <IconTrash />
                      </button>
                    </div>
                  </li>
            ...
            <form onSubmit={addHours}
              style={{ display:"grid", gridTemplateColumns:`${HOURS_ADD_FIELD_HALF_WIDTH_PX}px ${HOURS_FIELD_WIDTH_PX}px ${HOURS_YEAR_FIELD_WIDTH_PX}px 50px`, gap:10, alignItems:"center", marginTop:10 }}>
              <input type="number" step="0.1" placeholder="uren" value={newHoursInput} onChange={(e)=>setNewHoursInput(e.target.value)} style={{ ...baseField, width:HOURS_ADD_FIELD_HALF_WIDTH_PX }} disabled={!selectedPersonId} />
              ...
              <input type="number" placeholder="jaar" value={newYearInput} onChange={(e)=>setNewYearInput(e.target.value)} style={{ ...baseField, width:HOURS_YEAR_FIELD_WIDTH_PX }} disabled={!selectedPersonId} />
              <button type="submit" aria-label="Uren toevoegen"
                style={{ height:36, width:50, minWidth:50, background:COLORS.btnBg, border:`1px solid ${COLORS.btnBorder}`, color:COLORS.btnText, borderRadius:8, cursor:"pointer", fontWeight:800, fontSize:18, lineHeight:1 }}
                disabled={!selectedPersonId}
                onMouseOver={(e)=> (e.currentTarget.style.background = !selectedPersonId ? COLORS.btnBg : COLORS.btnHover)}
                onMouseOut={(e)=> (e.currentTarget.style.background = COLORS.btnBg)}>+</button>
            </form>
          </div>

          {/* --------- RECHTS – Personeel beheren --------- */}
          <div style={{ width:`min(${PERSON_CARD_MAX_WIDTH}px, 100%)`, background:COLORS.card, border:`1px solid ${COLORS.line}`, borderRadius:12, padding:16, display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ display:"grid", gridTemplateColumns:"6px 1fr", alignItems:"center", columnGap:8 }}>
              <div style={{ width:6, height:20, background:COLORS.primary, borderRadius:3 }} />
            ...
            <form onSubmit={addPerson}
              style={{ display:"grid", gridTemplateColumns:`${PERSON_FIELD_WIDTH_PX}px ${PERSON_FIELD_WIDTH_PX}px ${PERSON_FIELD_WIDTH_PX}px ${PERSON_FIELD_WIDTH_PX}px 50px`, gap:8, alignItems:"center" }}>
              ...
              <button type="submit" disabled={savingNewPerson} aria-label="Personeel toevoegen"
                style={{ height:36, width:50, minWidth:50, background:COLORS.btnBg, border:`1px solid ${COLORS.btnBorder}`, color:COLORS.btnText, borderRadius:8, cursor:"pointer", fontWeight:800, fontSize:16, lineHeight:1 }}
            ...
            {loadingPeople ? (
              ...
              <ul style={{ listStyle:"none", padding:0, margin:0 }}>
                {people.map((p) => (
                  <li key={p.id} onMouseEnter={()=>setHoveredAvatarId(p.id)} onMouseLeave={()=>setHoveredAvatarId(null)}
                    style={{ display:"grid", gridTemplateColumns:`28px ${PERSON_FIELD_WIDTH_PX}px ${PERSON_FIELD_WIDTH_PX}px ${PERSON_FIELD_WIDTH_PX}px ${PERSON_FIELD_WIDTH_PX}px auto`, gap:8, alignItems:"center", borderBottom:`1px solid ${COLORS.line}`, padding:"8px 0", position:"relative" }}>
                    {/* Avatar */}
                      ...
                      {p.avatar_url ? (
                        <>
                          ...
                          {hoveredAvatarId === p.id && (
                            <button aria-label="Avatar verwijderen" onClick={()=>askDeleteAvatar(p)} title="Verwijder avatar"
                              style={{ position:"absolute", inset:0, border:"none", background:"rgba(239,68,68,0.6)", color:"#fff", fontSize:18, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                              ×
                            </button>
                          )}
                        </>
                        ...
                        <>
                          <div onClick={()=> (document.getElementById(`file-${p.id}`) as HTMLInputElement | null)?.click()}
                            aria-label="lege avatar - klik om te uploaden" title="Klik om avatar te uploaden"
                            style={{ width:"100%", height:"100%", borderRadius:"50%", border:`1px solid ${COLORS.line}`, cursor:"pointer", position:"relative" }} />
                          {hoveredAvatarId === p.id && (
                            <button aria-label="Upload foto" onClick={()=> (document.getElementById(`file-${p.id}`) as HTMLInputElement | null)?.click()} title="Foto uploaden"
                              style={{ position:"absolute", inset:0, border:"none", background:"rgba(14,165,168,0.15)", color:COLORS.primary, fontSize:18, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                              +
                            </button>
                          )}
                    ...
                    <input value={p.surname ?? ""} onChange={(e)=>updatePersonField(p.id, "surname", e.target.value)} placeholder="achternaam" style={{ ...baseField, height:32, width:PERSON_FIELD_WIDTH_PX }} />
                    <input type="date" value={p.birthdate ?? ""} onChange={(e)=>updatePersonField(p.id, "birthdate", e.target.value)} placeholder="geboortedatum" style={{ ...baseField, height:32, width:PERSON_FIELD_WIDTH_PX }} />
                    <select value={p.status ?? (statusOptions[0] || "")} onChange={(e)=>updatePersonField(p.id, "status", e.target.value)} style={{ ...baseField, height:32, width:PERSON_FIELD_WIDTH_PX }}>
                      {statusOptions.map((opt) => (<option key={opt} value={opt}>{opt}</option>))}
                    </select>

                    <div style={{ display:"flex", gap:6, justifyContent:"flex-end" }}>
                      <button aria-label="Opslaan" title="Opslaan" onClick={()=>savePerson(p)}
                        style={{ background:"transparent", border:`1px solid ${COLORS.line}`, height:32, width:32, borderRadius:8, display:"inline-flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                        <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={COLORS.primary} strokeWidth={2.6} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                          <path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z" />
                          <polyline points="17 21 17 13 7 13 7 21" />
                      ...
                      <button aria-label="Verwijderen" title="Verwijderen" onClick={()=>askDeletePerson(p)}
                        style={{ background:"transparent", border:"none", height:32, width:32, borderRadius:8, display:"inline-flex", alignItems:"center", justifyContent:"center", cursor:"pointer" }}>
                        <IconTrash />
                      </button>
                    </div>
        ...
        {/* ========== RIJ 2: Admin1 (Feestdagen + Schoolvakanties, elk eigen jaar; vaste breedte) ========== */}
        <section
          style={{
            display:"grid",
        ...
        >
          {/* --------- LINKS – Feestdagen --------- */}
          <div style={{ width:`min(${FEESTDAG_CARD_WIDTH}px, 100%)`, background:COLORS.card, border:`1px solid ${COLORS.line}`, borderRadius:12, padding:16, display:"flex", flexDirection:"column", gap:14 }}>
            {/* Titel + Jaar (in de card zelf) */}
            <div style={{ display:"grid", gridTemplateColumns:"6px 1fr auto", alignItems:"center", columnGap:8 }}>
              ...
              <input type="number" value={yearPublic} onChange={(e)=>setYearPublic(parseInt(e.target.value || String(new Date().getFullYear()), 10) || new Date().getFullYear())}
                title="Jaar" aria-label="Jaar (feestdagen)" style={{ ...baseField, width:90, height:32 }} />
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"6px 1fr", alignItems:"center", columnGap:8, marginTop:4 }}>
              ...
              <h3 className={variableFont.className} style={{ margin: 0, fontSize: 16 }}>Feestdagen in {yearPublic}</h3>
            </div>
...
>
  ...
  >
    <IconTrash />
          ...
          <div style={{ width:`min(${SCHOOL_CARD_WIDTH}px, 100%)`, background:COLORS.card, border:`1px solid ${COLORS.line}`, borderRadius:12, padding:16, display:"flex", flexDirection:"column", gap:14 }}>
            {/* Titel + Jaar (in de card zelf) */}
            <div style={{ display:"grid", gridTemplateColumns:"6px 1fr auto", alignItems:"center", columnGap:8 }}>
              <div style={{ width:6, height:20, background:COLORS.primary, borderRadius:3 }} />
              <h2 className={variableFont.className} style={{ margin:0, fontSize:18 }}>Schoolvakanties toevoegen</h2>
              <input type="number" value={yearSchool} onChange={(e)=>setYearSchool(parseInt(e.target.value || String(new Date().getFullYear()), 10) || new Date().getFullYear())}
                title="Jaar" aria-label="Jaar (schoolvakanties)" style={{ ...baseField, width:90, height:32 }} />
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"6px 1fr", alignItems:"center", columnGap:8, marginTop:4 }}>
              <div style={{ width:6, height:18, background:COLORS.primary, borderRadius:3 }} />
              <h3 className={variableFont.className} style={{ margin: 0, fontSize: 16 }}>Schoolvakanties in {yearSchool}</h3>
            </div>
            ...
            {loadingSchool ? (
              ...
              const grouped = Array.from(map.entries()).map(([name, r]) => ({ name, start: r.start, end: r.end }))
